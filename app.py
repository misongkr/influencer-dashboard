...
TYPE_BADGE = {
    "시간형": {"bg": "#C4B5FD", "fg": "#312E81"},
    "파워형": {"bg": "#FCA5A5", "fg": "#7F1D1D"},
    "밸런스형": {"bg": "#D1D5DB", "fg": "#111827"},
}
...
def parse_period_from_filename(name: str):
    ...
    # 2) 월간: YYYYMM (6자리)
    m = re.search(r"(20\d{4}\d{2})(?!\d)", stem)  # YYYYMM
    if m:
        token = m.group(1)
        try:
...
def load_latest_prev_streamer_softcon_and_cat(data_dir):
...
def compute_overview_metrics(streamer_rank: pd.DataFrame, cat_stats_week: pd.DataFrame):
    ...
    kpis = {
        ...
        "평균 시청자(카테고리)": cat_avg_viewers,
        "총 방송시간": total_stream_time,
        "가중 평균 시청자(뷰어십/시간)": weighted_avg_viewers,
    }
...
st.sidebar.write("GIT_SHA:", git_sha)
st.sidebar.write("BASE_DIR:", str(BASE_DIR))
st.sidebar.write("DATA_ROOT:", str(DATA_ROOT))
st.sidebar.write("SELECTED_DATA_DIR:", str(data_dir))
st.sidebar.write("SELECTED_DATA_DIR_EXISTS:", data_dir.exists())
